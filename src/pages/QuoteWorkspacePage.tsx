...
function QuoteWorkspacePage() {
  ...
  return (
    <>
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]" data-print-hidden="true">
        {/* ══════════════════════════════════════════════
            LEFT COLUMN: Editor form with collapsible sections
            ══════════════════════════════════════════════ */}
        <div className="grid gap-4">
          {/* ── Top bar ── */}
          <div className="sticky top-0 z-10 -mx-1 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/70 bg-card/95 px-5 py-3 shadow-sm backdrop-blur-sm">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {hasUnsavedChanges && (
                <span className="text-xs font-medium text-amber-600">{t("quote.footer.idle")}</span>
              )}
              <Button
                type="button"
                variant="outline"
          ...
          {/* ── Editor form ── */}
          <form id="quote-workspace-form" className="grid gap-4" onSubmit={handleSubmit}>

            {/* ──────── Section 1: Client ──────── */}
            <CollapsibleSection
              icon={RiUserLine}
              title={t("quote.sections.client")}
              subtitle={form.customer_name || form.customer_company || undefined}
              defaultOpen
            >
              ...
              <CollapsibleOptional
                label={t("quote.sections.moreContactDetails")}
                hasContent={!!(form.customer_phone || form.customer_address)}
              >
                <div className="grid gap-4 sm:grid-cols-2">
            ...
            {/* ──────── Section 2: Work to quote ──────── */}
            <CollapsibleSection
              icon={RiFileTextLine}
              title={t("quote.sections.workToQuote")}
              subtitle={form.job_summary ? form.job_summary.slice(0, 60) + (form.job_summary.length > 60 ? "..." : "") : undefined}
              defaultOpen
            >
              <div className="grid gap-4">
            ...
            <CollapsibleSection
              ...
              title={t("quote.sections.pricing")}
              badge={form.line_items.length > 0 ? String(form.line_items.length) : undefined}
              defaultOpen
            >
              {/* Catalog insert from library */}
                ...
                <div className="rounded-lg border border-border/60 bg-card p-4">
                  ...
                  <div className="mt-3 grid gap-3 md:grid-cols-2">
                    ...
                    {selectedPricedItem?.pricing_mode === "volume_direct" ? (
                      <div className="grid gap-3 md:col-span-2 md:grid-cols-[minmax(0,1fr)_0.55fr]">
                  ...
                  {selectedPricedItem ? (
                    <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border/60 bg-secondary/30 px-4 py-3">
                      <div>
              ...
              {form.line_items.length === 0 ? (
                ...
                <div className="grid gap-3">
                  {form.line_items.map((item, index) => (
                    ...
                    >
                      {/* Row 1: description + numbers */}
                      <div className="grid gap-3 sm:grid-cols-[minmax(0,1.5fr)_0.5fr_0.5fr_0.6fr]">
                        ...
                        <FieldBlock label={t("quote.fields.unitPrice.label")}>
                          <Input
                            type="number"
                            min="0"
                            step="0.01"
                      ...
                      {/* Row 2: status + line total + actions */}
                      <div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-border/40 pt-3">
                        <div className="flex items-center gap-3">
                          <label className="inline-flex items-center gap-2">
                            ...
                            />
                            <span className="text-xs text-muted-foreground">{t("quote.lineItems.needsReview")}</span>
                          </label>
                          {(item.needs_review || parseMoneyToCents(item.unit_price) === null) && (
            ...
            {/* ──────── Section 4: Before sending ──────── */}
            <CollapsibleSection
              icon={RiStickyNoteLine}
              title={t("quote.sections.beforeSending")}
              defaultOpen={!!(form.assumptions || form.payment_terms || form.notes)}
            >
              <div className="grid gap-4">
        ...
        {/* ══════════════════════════════════════════════
            RIGHT COLUMN: Sticky sidebar (summary + chat)
            ══════════════════════════════════════════════ */}
        <div className="xl:sticky xl:top-6 xl:max-h-[calc(100vh-3rem)] xl:self-start">
          <Card className="flex flex-col overflow-hidden xl:max-h-[calc(100vh-3rem)]">
            ...
            <div className="border-b border-border/60 bg-secondary/20 px-5 py-4">
              ...
              <div className="flex items-baseline justify-between gap-3">
                ...
                <span className="font-heading text-2xl font-bold tabular-nums tracking-tight">
                  {formatCurrency(preview.total, form.currency, locale)}
                </span>
              </div>
              {/* Subtotal + tax on one row */}
              <div className="mt-1 flex items-center justify-between gap-3 text-xs text-muted-foreground">
                <span>
                  {t("quote.subtotal")}: {formatCurrency(preview.subtotal, form.currency, locale)}
                </span>
                <span>
                  {t("quote.tax", { value: formatPercent((parseNumberInput(form.tax_rate) ?? 0) / 100, locale) })}:{" "}
                  {formatCurrency(preview.tax, form.currency, locale)}
                </span>
              </div>
              ...
              <div className="mt-3">
                {reviewItems.length > 0 ? (
                  <div className="flex items-center gap-2 rounded-lg border border-amber-300/60 bg-amber-50 px-3 py-2 text-xs text-amber-900">
                    ...
                    <span>{t("quote.review.needsAttention", { count: reviewItems.length })}</span>
                  </div>
                ) : form.line_items.length > 0 ? (
                  <div className="flex items-center gap-2 rounded-lg border border-emerald-300/60 bg-emerald-50 px-3 py-2 text-xs text-emerald-900">
                    <RiCheckLine className="size-3.5 shrink-0" />
                    <span>{t("quote.review.readyToPrint")}</span>
                  </div>
                ) : null}
              </div>
            ...
            <div className="flex min-h-0 flex-1 flex-col">
              <div className="flex items-center gap-2 px-5 pt-4 pb-2">
                <RiChat3Line className="size-3.5 text-primary" />
                <span className="text-sm font-semibold tracking-tight text-foreground">{t("quote.tabs.chat")}</span>
              </div>
              ...
              <div
                ref={chatScrollRef}
                className="flex min-h-[10rem] flex-1 flex-col gap-2.5 overflow-y-auto px-5 py-3"
              >
              ...
              <div className="border-t border-border/60 px-5 py-3">
                <form className="grid gap-2.5" onSubmit={handleChatSubmit}>
                  {/* Recording indicator */}
...
function CollapsibleSection({
  ...
  return (
    <section className="rounded-xl border border-border/60 bg-card shadow-sm">
      ...
      {isOpen ? (
        <div className="grid gap-4 border-t border-border/40 px-5 pb-5 pt-4">
          {children}
        </div>
      ) : null}
    </section>
